...
const Result = ({ pointer }) => {
    ...
    return (
        <div>
            <h2 className='heading text-gray-200'>{`Your pointer is ${res} !`}</h2>
            <div className='h-full w-full items-center flex justify-center '>
               <img src = {imgUrl} className='h-3/4 w-3/4  md:h-2/4 md:w-2/4 '/>
            </div>
            <p className='text-xl text-gray-200' style={{marginTop: '10px', textAlign: 'center'}}>{message}</p>
        </div>
    )
}
